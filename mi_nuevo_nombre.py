...
with col_plot:
    sol = solve_ivp(
        model, [0, 10], [y0_val], args=(k_val,), t_eval=np.linspace(0, 10, 200)
    )
    ...
    # Ajustes estéticos para que la gráfica sea cómoda y legible
    fig.update_layout(
...
with col_data:
    ...
    if st.session_state.history:
        for i, item in enumerate(reversed(st.session_state.history)):
            with st.expander(
                f"Simulación #{len(st.session_state.history) - i}"
            ):
                st.write(f"**K:** {item['K']}")
